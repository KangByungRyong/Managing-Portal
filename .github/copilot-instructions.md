...
### 사이드바 (Side-In 패널) 규격 (필수 준수)

모든 Drill Down 사이드바는 **너비 40% 고정**을 사용한다.

| 항목      | 기준값                                        |
| --------- | --------------------------------------------- |
| 너비      | `w-[40%]` (반응형 다중 클래스 사용 금지)      |
| 위치      | `fixed right-0 top-0 h-full`                  |
| 트랜지션  | `transform transition-transform duration-500` |
| 열림 상태 | `translate-x-0`                               |
| 닫힘 상태 | `translate-x-full`                            |

**금지 패턴**:

```tsx
// ❌ 금지: 반응형 다중 너비 클래스 사용
className = "... w-full sm:w-[86%] lg:w-[60%] xl:w-[50%] ...";
```
...
**적용 대상 컴포넌트** (`src/app/components/`):

- `Gijiguk5GSidebar.tsx`, `GijigukKpiSidebar.tsx`, `GijigukEquipDrilldownSidebar.tsx`, `GijigukCityDetailSidebar.tsx`
- `RepeaterKpiSidebar.tsx`, `RepeaterEquipDrilldownSidebar.tsx`, `RepeaterCityDetailSidebar.tsx`
- `StationListSidebar.tsx`, `StationDetailSidebar.tsx`, `IssueDetailSidebar.tsx`
- `home/HomeDrilldownSidebar.tsx`, `home/StationKpiSidebar.tsx`
