...
interface Gijiguk5GSidebarProps {
  isOpen: boolean;
  onClose: () => void;
  rows: GijigukRow[];
}
...
export function Gijiguk5GSidebar({
  isOpen,
  onClose,
  rows,
}: Gijiguk5GSidebarProps) {
  const stateTotalsMap: Record<string, number> = {};
  rows.forEach((r) => {
    const city5GTotal = Object.values(r.fiveG).reduce(
      (a, v) => a + (v ?? 0),
      0,
    );
    stateTotalsMap[r.state] =
      (stateTotalsMap[r.state] ?? 0) + city5GTotal;
  });
  ...
  const fiveGTypeTotals = GIJIGUK_5G_EQUIP.map((type) => ({
    type,
    value: rows.reduce((sum, row) => sum + (row.fiveG[type] ?? 0), 0),
  }))
    .filter((d) => d.value > 0)
    .sort((a, b) => b.value - a.value);

  const maxTypeValue =
    fiveGTypeTotals.length > 0
      ? Math.max(...fiveGTypeTotals.map((d) => d.value))
      : 0;

  const total5G = fiveGTypeTotals.reduce((a, d) => a + d.value, 0);
      ...
      >
        ...
        >
          <div className="flex items-center gap-2">
            ...
            />
            <h2 className="text-lg font-bold text-gray-900">5G 장비 운용 현황</h2>
            <span className="text-sm text-gray-500 font-normal ml-1">
              총 {total5G.toLocaleString()}대
            </span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto h-[calc(100%-73px)] space-y-5">
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="space-y-3">
              ...
              {fiveGTypeTotals.map((item) => {
                const widthPercent =
                  maxTypeValue > 0
                    ? Math.max(6, (item.value / maxTypeValue) * 100)
                    : 0;
                return (
