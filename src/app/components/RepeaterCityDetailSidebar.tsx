...
interface RepeaterCityDetailSidebarProps {
  isOpen: boolean;
  onClose: () => void;
  rows: RepeaterRow[];
  selectedCityStat: RepeaterCityStats | null;
}
...
type RepeaterRecordKey = "fiveG" | "lte" | "wcdma" | "wibro" | "idong";

const CATEGORY_CONFIG: Array<{
  label: string;
  recordKey: RepeaterRecordKey;
  equipKeys: readonly string[];
  color: string;
}> = [
  { label: "5G", recordKey: "fiveG", equipKeys: REPEATER_5G_EQUIP, color: "#7C3AED" },
  { label: "LTE", recordKey: "lte", equipKeys: REPEATER_LTE_EQUIP, color: "#2563EB" },
  { label: "WCDMA", recordKey: "wcdma", equipKeys: REPEATER_WCDMA_EQUIP, color: "#16A34A" },
  { label: "WiBro", recordKey: "wibro", equipKeys: REPEATER_WIBRO_EQUIP, color: "#0EA5E9" },
  { label: "이동형", recordKey: "idong", equipKeys: REPEATER_IDONG_EQUIP, color: "#EA580C" },
];

const PALETTE = ["#2563EB", "#7C3AED", "#059669", "#EA580C", "#EF4444", "#DB2777", "#0891B2", "#4F46E5", "#65A30D", "#0D9488"];

function colorAt(index: number) {
  return PALETTE[index % PALETTE.length];
}

function DonutChart({ title, unit, total, slices }: { title: string; unit: string; total: number; slices: DonutSlice[] }) {
  const SIZE = 160;
  const R = 54;
  const CX = SIZE / 2;
  const CY = SIZE / 2;
    ...
    <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
      ...
      <div className="flex items-start gap-4">
        <svg width={SIZE} height={SIZE} className="shrink-0">
          ...
          <text x={CX} y={CY - 6} textAnchor="middle" style={{ fontSize: 18, fontWeight: 700, fill: "#111827", fontFamily: "monospace" }}>
        ...
        <div className="flex-1 space-y-1.5 min-w-0 max-h-[160px] overflow-auto pr-1">
          {filtered.length === 0 && <div className="text-[11px] text-gray-400">표시할 장비 데이터가 없습니다.</div>}
          {filtered.map((slice) => {
...
function CategoryBarChart({ title, items, accentColor }: { title: string; items: BarItem[]; accentColor: string }) {
  ...
  return (
    <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
      ...
      <div className="space-y-2.5 max-h-[260px] overflow-auto pr-1">
        {items.length === 0 && <div className="text-[11px] text-gray-400">표시할 장비 형태 데이터가 없습니다.</div>}
        {items.map((item) => {
          ...
          return (
            <div key={item.label} className="space-y-1">
              <div className="flex items-center justify-between gap-2 text-[11px]">
                ...
                <span className="font-mono font-semibold text-gray-800">{item.value.toLocaleString()}</span>
              </div>
              <div className="h-2.5 rounded-full bg-gray-100 overflow-hidden">
                <div className="h-full rounded-full" style={{ width: `${widthPercent}%`, backgroundColor: item.color || accentColor }} />
              </div>
            </div>
          );
        })}
...
export function RepeaterCityDetailSidebar({ isOpen, onClose, rows, selectedCityStat }: RepeaterCityDetailSidebarProps) {
  ...
  const categorySeries = useMemo(() => {
    return CATEGORY_CONFIG.map((category) => {
      const items = category.equipKeys
        .map((key, index) => {
          const value = targetRows.reduce((sum, row) => sum + (row[category.recordKey][key] ?? 0), 0);
          return {
            label: key,
            value,
            color: colorAt(index),
  ...
  return (
    <>
      ...
      <div
        className={`fixed right-0 top-0 h-full w-full sm:w-[86%] lg:w-[60%] xl:w-[50%] bg-white shadow-2xl z-[321] transform transition-transform duration-500 ${
      ...
      >
        ...
        >
          <div className="min-w-0">
            <h2 className="text-lg font-bold text-gray-900">중계기 세부 장비 현황</h2>
        ...
        <div className="p-6 overflow-y-auto h-[calc(100%-73px)] space-y-4">
          <DonutChart title="장비 Category 현황" unit="대" total={categoryDonut.total} slices={categoryDonut.slices} />
