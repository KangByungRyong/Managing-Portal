...
interface GijigukEquipDrilldownSidebarProps {
  isOpen: boolean;
  onClose: () => void;
  rows: GijigukRow[];
  equipLabel: string;
  equipKeys: readonly string[];
  recordKey: "fiveG" | "lte" | "wcdma" | "lora";
}
...
export function GijigukEquipDrilldownSidebar({
  isOpen,
  onClose,
  rows,
  equipLabel,
  equipKeys,
  recordKey,
}: GijigukEquipDrilldownSidebarProps) {
  const stateTotalsMap: Record<string, number> = {};
  rows.forEach((r) => {
    const equipRecord = r[recordKey] as Partial<Record<string, number>>;
    const stateTotal = Object.values(equipRecord).reduce((a, v) => a + (v ?? 0), 0);
    stateTotalsMap[r.state] = (stateTotalsMap[r.state] ?? 0) + stateTotal;
  });
  ...
  const maxTypeValue = typeTotals.length > 0 ? Math.max(...typeTotals.map((d) => d.value)) : 0;
  const totalEquip = typeTotals.reduce((a, d) => a + d.value, 0);

  return (
    <>
      {isOpen && (
        <div className="fixed inset-0 bg-black/30 z-[300] transition-opacity duration-500" onClick={onClose} />
      )}
        ...
        <div className="p-6 overflow-y-auto h-[calc(100%-73px)] space-y-5">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            <div className="flex items-center gap-1.5 mb-4">
              ...
              <h3 className="text-sm font-bold text-gray-700">광역시도별 {equipLabel} 장비 현황</h3>
            </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            <div className="flex items-center gap-1.5 mb-4">
              <div className="w-0.5 h-3.5 rounded" style={{ backgroundColor: "var(--region-primary)" }} />
              <h3 className="text-sm font-bold text-gray-700">{equipLabel} 장비 형태별 구성</h3>
            </div>

            <div className="space-y-3">
              {typeTotals.length === 0 && <div className="text-xs text-gray-400">표시할 {equipLabel} 장비 데이터가 없습니다.</div>}
              {typeTotals.map((item) => {
